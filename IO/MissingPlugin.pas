...
const
  RegisteredAttributes: array [0 .. 13] of string = ('weight', 'color',
    'symmetry', 'color_speed', 'coefs', 'chaos', 'plotmode', 'opacity', 'post',
    'var', 'var1', 'var_color', 'name', 'linear3D');

var
  MissingPluginList: TStringList;
  Parsing: boolean;
...
begin
  for i := 0 to Length(RegisteredAttributes) - 1 do
    if attr = RegisteredAttributes[i] then
      exit;
...
begin
  ...
  if (cp.used_plugins.Count > 0) then
  begin
  ...
  begin
    statusPanelText := Format(TextByKey('main-status-loadingerrorcount'),
      [MissingPluginList.Count]);

    for i := 0 to MissingPluginList.Count - 1 do
      str := str + #13#10 + '  - ' + MissingPluginList[i];
    ErrorMessageString := Format(TextByKey('main-status-morepluginsneeded'),
      [cp.name, str2]) + str;
    LoadForm.Output.Text := LoadForm.Output.Text + ErrorMessageString +
      #13#10#13#10;
    Result := false;
  end
  else
...
begin
  if (ErrorMessageString = '') or (not WarnOnMissingPlugin) then
    exit;
  MessageBox($00000000, PChar(ErrorMessageString), PChar('Apophysis'),
