...
begin
  if FindFirst(ExtractFilePath(Application.ExeName) + 'Languages\*.xml',
    faAnyFile, searchResult) = 0 then
  begin
    repeat
      AvailableLanguages.Add(ExtractFilePath(Application.ExeName) + 'Languages\'
        + searchResult.name);
...
begin
  ...
  Add('common-confirmexit',
    'Do you really want to exit? All unsaved data will be lost!');
  Add('common-confirmdelete',
  ...
  Add('common-trace-notenoughmemory',
    'ERROR: Not enough memory for this render!');
  Add('common-statistics-title-oneslice', 'Render Statistics:');
  Add('common-statistics-title-multipleslices',
    'Render Statistics for the last slice:');
  ...
  Add('common-statistics-averagespeed',
    'Average speed: %n iterations per second');
  ...
  Add('editor-tab-transform-resethint',
    'Reset all vectors to default position');
  Add('editor-tab-transform-rectangular', 'Rectangular');
  Add('editor-tab-transform-rectangularhint',
    'Show vectors in rectangular (cartesian) corrdinates');
  ...
  Add('editor-tab-transform-resetposthint',
    'Reset all post-transform vectors to default position');
  ...
  Add('render-resourceusage-infotext',
    'The render process will use %u MB of %u MB available physical memory');
  Add('render-resourceusage-infotext2',
    'Apophysis will try to use %u processor cores (%u available) - change this in the options');
  ...
  Add('render-status-saveerror-message1',
    'An error occured while saving the image:');
  Add('render-status-saveerror-message2',
  ...
  Add('render-status-notenoughmemory1',
    'You do not have enough memory for this render. Do you want to continue anyway?');
  Add('render-status-notenoughmemory2',
    'You do not have enough memory for this render. Please use a lower Maximum memory setting. Do you want to ignore this problem and continue?');
  ...
  Add('render-status-maxmemorytoosmall',
    'Maximum memory value is too small. Do you want to continue anyway?');
  ...
  Add('render-status-log-largepng-message1',
    '*** WARNING *** You have selected PNG format and an image size which exceeds 20 megapixels');
  Add('render-status-log-largepng-message2',
    'PNG format with extreme high-resolution images is not recommended!');
  Add('render-status-log-largepng-message3',
  ...
  Add('options-restartnotice',
    'You must restart Apophysis 7x to make your changes have effect');
  ...
  Add('options-tab-general-showextendedstatistics',
  ...
  Add('options-tab-general-enablemissingpluginswarning',
    'Warn on missing extensions ');
  ...
  Add('options-tab-general-multithreadedpreview',
    'Multithreaded preview (experimental!)');
  Add('options-tab-general-multithreadedpreview-warning',
    'WARNING: multithreaded previews are an experimental and might cause problems while using Apophysis 7X. Please disable this setting in case of problems.');
  ...
  Add('options-tab-editor-alwaysshowbothtransformtypes',
    'Always show both transform types ');
  ...
  Add('options-tab-display-usesmallthumbs',
    'Use small thumbnails (like Apophysis 2.09) ');
  ...
  Add('options-tab-environment-rememberlastopen',
    'Remember last open parameters ');
  ...
  Add('save-status-alreadyexists',
    '"%s" in "%s" already exists. Do you want to replace it?');
  Add('save-status-alreadyexists2',
  ...
  Add('main-menu-flame-resumeunfinished',
    'Resume unfinished render process...');
  ...
  Add('main-status-nohelpfile',
    'Please specify a help file path in the options dialog first.');
  Add('main-status-variationsorvariables', 'variations or variables');
  Add('main-status-plugins', 'extensions');
  Add('main-status-pluginpath-ioerror',
    'Failed to write the setting for the extension directory. Apophysis will use the default setting.');
  Add('main-status-noloadingerrors', 'Flame loaded without errors');
  Add('main-status-loadingerrorcount', '%d errors in flame');
  Add('main-status-morepluginsneeded',
    'The flame "%s" requires the following additional %s:');
...
begin
  ...
  begin
    if LowerCase(language[i].key) = LowerCase(key) then
    begin
  ...
  for i := 0 to tokenCount - 1 do
  begin
    if LowerCase(defaultlanguage[i].key) = LowerCase(key) then
    begin
...
procedure TParser.ListXmlScannerStartTag(Sender: TObject; TagName: string;
  Attributes: TAttrList);
begin
...
procedure TParser.ListXmlScannerEmptyTag(Sender: TObject; TagName: string;
  Attributes: TAttrList);
var
  lastIndex: integer;
begin
...
const
  root: string = 'stringtable';
var
  key, tn: string;
  i: integer;
begin
  for i := 0 to self.parentTagnames.Count - 1 do
  begin
    tn := self.parentTagnames.Strings[i];
    if not(tn = '') and not(tn = root) then
      key := key + tn + '-';
  end;
